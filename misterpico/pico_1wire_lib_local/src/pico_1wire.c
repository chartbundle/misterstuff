...
extern void loop_1();
extern void loop_1_delay(uint16_t);
...
bool pico_1wire_reset_bus(pico_1wire_t *ctx)
{
	...
	loop_1_delay(RESET_PULSE_TX_MIN_LEN);
...
//	sleep_us(RESET_PULSE_RX_MIN_LEN - 15 - i);
	loop_1_delay(RESET_PULSE_RX_MIN_LEN - 15 - i);
...
int pico_1wire_get_temperature(pico_1wire_t *ctx, uint64_t addr, uint16_t *temperature)
{
	uint8_t scratch[9];
	int16_t temp_read;
	uint16_t temp;
	...
	case FAMILY_CODE_DS1825:
	case FAMILY_CODE_DS28EA00:
		temp =  100*temp_read / 16;
		break;

	case FAMILY_CODE_DS18S20:
		count_remain = scratch[6];
		count_per_degree = scratch[7];
		// Temp_read is in half degrees
		temp = ((temp_read*50) - 25) + (100*(count_per_degree - count_remain) / count_per_degree);
		break;

	default:
		temp = 100*temp_read / 16; /* Best quess... */
